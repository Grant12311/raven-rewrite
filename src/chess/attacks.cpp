...
int isCheck(Position *pos) {
	if (pos->tomove == WHITE)
		return isAttacked(pos, pos->Wkingpos, !pos->tomove);
	else
...
int isAttacked(Position *pos, int square, int colour) {
	...
	// pawn attacks
	if (colour == WHITE)
		BBattacks = pawn_attacksB(BBpiece);
	else
		BBattacks = pawn_attacksW(BBpiece);
...
U64 BBpawnSinglePushW(U64 BBwpawns, U64 BBunoccupied) { return northOne(BBwpawns) & BBunoccupied; }
...
U64 BBpawnSinglePushB(U64 BBbpawns, U64 BBunoccupied) { return southOne(BBbpawns) & BBunoccupied; }
...
U64 pawn_attacksW(U64 BBpiece) { return BBpawnEastAttacksW(BBpiece) | BBpawnWestAttacksW(BBpiece); }
U64 pawn_attacksB(U64 BBpiece) { return BBpawnEastAttacksB(BBpiece) | BBpawnWestAttacksB(BBpiece); }
U64 king_attacks(U64 BBpiece) { return libchess::movegen::king_moves(BBpiece); }
U64 knight_attacks(U64 BBpiece) { return libchess::movegen::knight_moves(BBpiece); }
U64 rook_attacks(U64 BBpiece, U64 BBocc) { return libchess::movegen::rook_moves(BBpiece, BBocc); }
U64 bishop_attacks(U64 BBpiece, U64 BBocc) { return libchess::movegen::bishop_moves(BBpiece, BBocc); }
U64 queen_attacks(U64 BBpiece, U64 BBocc) { return libchess::movegen::queen_moves(BBpiece, BBocc); }
